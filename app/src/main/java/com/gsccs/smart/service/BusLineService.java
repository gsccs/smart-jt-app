...
import com.gsccs.smart.model.BusLineMessage;
import com.gsccs.smart.model.ErrorStatus;
import com.gsccs.smart.model.PoiMessage;
...
//    private Map<String,Parcelable> busMap = null;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.d("testBusLine","onStartCommand*******");
        if(startId == 1){
            if(null != intent){
                ...
                if(errorStatus.getIsError()){
                    ...
                    Log.d("BusLineSearch",search);
                    if(-1 != search.indexOf(";")){      //是否附件有多条公交线路
    ...
    public void startSearch(){
        Log.d("BusLineService","startSearch");
        if(0 != MESSAGE_FLAG){  //单条公路线路
            searchLine(busLineItems.get(busLineItems.size()-MESSAGE_FLAG).get("LineNumber").toString());
            MESSAGE_FLAG --;
        }else{
            //发广播
            Log.d("BusLineService","BroadcastSender");
            ...
            Log.d("busLineBroadcast","sendBroadcast");
...
//                        Log.i("testAki48475757",MESSAGE_FLAG+1 + "*/*/*/*/" + busLineItems.get(busLineItems.size()-(MESSAGE_FLAG+1)).get("GoBusLineMessage").toString());
                        ...
                        for (BusLineItem line: lineItems) {
                            Log.d("lineSearch", busLineItems.size()
